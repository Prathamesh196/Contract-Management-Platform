...
const FieldPalette: React.FC<FieldPaletteProps> = ({ onAddField }) => {
  ...
  return (
    <div className="w-full h-full bg-white border-r border-slate-200 flex flex-col shrink-0">
      <div className="p-6 border-b border-slate-100">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Field Palette</h3>
        <p className="text-[11px] text-slate-500 leading-tight">Tap to add components to your document blueprint.</p>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {tools.map((tool) => (
          <button
            ...
            className="w-full flex items-start space-x-4 p-4 text-left border border-slate-200 rounded-xl hover:border-blue-500 hover:bg-blue-50 hover:shadow-md transition-all group active:scale-[0.98]"
          >
      ...
      <div className="p-6 bg-slate-50 border-t border-slate-100">
        <div className="bg-blue-600/10 p-4 rounded-xl border border-blue-100 hidden sm:block">
            <p className="text-[11px] text-blue-700 font-medium leading-relaxed">
              <strong>Tip:</strong> Edit labels directly on the canvas once components are added.
            </p>
        </div>
      </div>
    </div>
